...
class TestOpenAICUABrowserResponsesAPI:
    ...
    async def test_cua_executes_actions_with_playwright(
        ...
        type_response = Mock(
            id="resp_2", 
            computer_call=Mock(id="call_2", type="type", text="Hello World")
        )
        done_response = Mock(id="resp_3", computer_call=None)
        
        mock_openai_client.responses.create.side_effect = [
            click_response, done_response,  # For click test
            type_response, done_response,    # For type test
        ]
    ...
    async def test_previous_response_id_chains_turns(
        ...
        responses = [
            Mock(id="resp_1", computer_call=Mock(id="call_1", type="click", coordinates={"x": 50, "y": 50})),
            Mock(id="resp_2", computer_call=Mock(id="call_2", type="click", coordinates={"x": 100, "y": 100})),
