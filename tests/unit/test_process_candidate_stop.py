...
class BrowserSpy:
    ...
    def read_profile_text(self) -> str: return "John Doe"
    def focus_message_box(self) -> None: ...
    def fill_message(self, text: str) -> None: ...
    def send(self) -> None: self.sent = True
...
class StopTrue:
    def is_stopped(self) -> bool: return True
...
def test_process_respects_stop_before_navigate():
    ...
    pc = ProcessCandidate(evaluate=eval_use, send=send, browser=b, seen=type("S", (), {"is_seen": lambda *_: False, "mark_seen": lambda *_: None})(), logger=log, stop=StopTrue())
