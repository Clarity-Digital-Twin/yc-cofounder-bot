...
def test_model_resolution():
    ...
    if decision_resolved:
        ...
        print(f"   ❌ DECISION_MODEL_RESOLVED not set")
        return False
    
    if cua_resolved:
        print(f"   ✅ CUA_MODEL_RESOLVED = {cua_resolved}")
    else:
        print(f"   ⚠️ CUA_MODEL_RESOLVED not set (may not have access)")
    ...
    if adapter.model == decision_resolved:
        ...
        print(f"   ❌ Decision adapter not using resolved model")
    ...
    if cua_resolved:
        ...
        try:
            ...
            if cua.model == cua_resolved:
                ...
                print(f"   ❌ CUA adapter not using resolved model")
    ...
    if "gpt-5" in decision_resolved.lower():
        if "thinking" in decision_resolved.lower():
            print(f"   ✅ Got preferred GPT-5 thinking model!")
        else:
            print(f"   ✅ Got GPT-5 model (non-thinking variant)")
    elif "gpt-4" in decision_resolved.lower():
        print(f"   ⚠️ Using GPT-4 fallback (no GPT-5 available on this account)")
    else:
