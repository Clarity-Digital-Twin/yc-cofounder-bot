...
class BrowserDebugger:
    ...
    def check_element(self, page: Any, selector: str) -> dict[str, Any]:
        ...
        try:
            if hasattr(page, "locator"):
                ...
                if count > 0:
                    ...
                    result = {"selector": selector, "found": False, "count": 0}

                self.log_action("check_element", result)
                return result
        except Exception as e:
            error_result = {"selector": selector, "error": str(e)}
            self.log_action("check_element_error", error_result)
            return error_result
        return {"selector": selector, "error": "No page methods available"}

    def _get_stack_trace(self) -> list[dict[str, Any]]:
        """Get simplified stack trace for debugging"""
        import traceback

        stack = traceback.extract_stack()[:-2]  # Exclude this function
        return [
            {"file": frame.filename.split("/")[-1], "line": frame.lineno, "function": frame.name}
            for frame in stack[-5:]  # Last 5 frames
        ]

    def dump_page_info(self, page: Any) -> dict[str, Any]:
...
def wrap_browser_for_debugging(browser_instance: Any, debugger: BrowserDebugger) -> Any:
    ...
    def debug_fill_message(text: str) -> Any:
